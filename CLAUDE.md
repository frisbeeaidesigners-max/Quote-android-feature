...
- V5 picker variant — второй стиль fullscreen quote-picker'а (`QuoteV5FullScreenContent.kt`), toggleable per-session через Switch в Profile. Та же FSM/controller/IME/replyContext что у V4, отличается шапкой (`HeaderConfig.Custom` `titleAlignment=LEFT`, BACK + «Сохранить») и нижней зоной (MessagePanel-style BottomStrip + floating PopoverCard). `LocalQuotePickerVariant` лежит на `AppContainer.quotePickerVariant`, дефолт V4. В `MainActivity` invocation picker'а обёрнут в `Box.clickable(indication=null) {}` — tap-barrier против пропуска тапов до MessagePanel'а под ним. `HeaderHost` принимает опциональный `backgroundColorOverride: Color?` (V5 передаёт `appSurface01(isDark)`, чтобы header сливался с picker'ом — дефолт DSBrand'а это `backgroundBase`).
- V5 Reply/Link tabs — `SegmentedControl(«Ответ» / «Ссылка»)` под BottomStrip'ом. Tab «Ответ» — текущий V5-флоу (selectable бабл + popover). Tab «Ссылка» — статичный MY `LinkBubbleView` с mock-данными Google Meet (`title`, `description`, `domain`, `url`, `time`, `sendingState=READ`) поверх `BackgroundPatternView` + `Arrangement.Bottom` + `Spacer(bottomSpacerDp)` (тот же animated DP, что у вкладки «Ответ», бабл едет вверх синхронно с popover'ом). Reply-блок LinkBubble всегда видим: `replyText` = `extractQuote(message, s, e)` из `snapshotRange` либо `replyPreviewText(message)`. `showQuoteIcon = snapshotRange != null` → маленькая quote-индикаторная иконка в reply-блоке (см. components: `LinkBubbleView.showQuoteIcon`). LinkBubble `message`-поле = draft текста MessagePanel'а — VM держит `panelDraftText: StateFlow<String>`, ChatDetailScreen вешает `TextWatcher` на `findFirstEditText()` (DisposableEffect-cleanup), MainActivity collect'ит и пробрасывает через `QuotePickerFullScreen.draftText`. BottomStrip swap по табу через `AnimatedContent` fade 150мс: `BottomStripLink` (link-chain icon `basicColor55`, «Прикрепленная ссылка» + URL, без popover-FSM но с popover-toggle тапом). На вкладке «Ссылка» popover = `LinkPopoverCard` с двумя декоративными пунктами «Общая информация» / «Главное сообщение» (selection state `linkPopoverSelection`, чекмарк `done` цвета `DSColors.success`, тап не закрывает popover). `LaunchedEffect(selectedTab)` коорд'ит переходы: на Tab 1 entry — `snapshotRange = live ?: existing`, `menuState = INITIAL_WITH_QUOTE` (если есть snapshot и не MINIMAL), `clearSelectionRef.invoke()`, `tvRef.clearFocus()`. На переход «1 → 0» (трекается через `previousTab`) — `restoreSelectionRef.invoke(s, e)` восстанавливает подсветку фрагмента. Popover форс'ится open на любом переключении вкладок. `restoreSelectionRef: MutableState<((Int, Int) -> Unit)?>` — новый ref в `QuoteBubblePreview` параллельно `selectAllRef`, делает `setMessageTextSelectable + view.post { selectMessageTextRange(s, e) }` под `suppressOnStart` guard'ом; V4 не нуждается, передаётся опционально.
